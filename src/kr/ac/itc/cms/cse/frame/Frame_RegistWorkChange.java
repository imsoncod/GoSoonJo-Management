...
public class Frame_RegistWorkChange extends JFrame implements ActionListener{

	private Container Frame_RegistWorkChange = getContentPane();
	...
	private void setRegistWorkChangeFrame() {
		Frame_RegistWorkChange.setBackground(Color.WHITE);
		
		label_workdate = new JLabel("근무 일자");
		label_workdate.setBounds(31, 62, 57, 15);
		Frame_RegistWorkChange.add(label_workdate);
		...
		Frame_RegistWorkChange.add(combo_year);
		...
		Frame_RegistWorkChange.add(combo_month);
		...
		Frame_RegistWorkChange.add(combo_starthour);
		...
		Frame_RegistWorkChange.add(label_starthour);
		...
		Frame_RegistWorkChange.add(combo_startminute);
		...
		Frame_RegistWorkChange.add(label_startminute);
		
		label_endwork = new JLabel("종료 시간");
		label_endwork.setBounds(31, 154, 57, 15);
		Frame_RegistWorkChange.add(label_endwork);
		...
		Frame_RegistWorkChange.add(combo_endhour);
		...
		Frame_RegistWorkChange.add(label_endhour);
		...
		Frame_RegistWorkChange.add(combo_endminute);
		...
		Frame_RegistWorkChange.add(label_endminute);
		
		label_timepay = new JLabel("시급");
		label_timepay.setBounds(57, 201, 31, 15);
		Frame_RegistWorkChange.add(label_timepay);
				
		Oracle_DAO Oracle = new Oracle_DAO();
		txt_timepay = new JTextField(Oracle.OracleGetData("userinfo", Frame_Login.email, "usertimepay"));
		...
		Frame_RegistWorkChange.add(txt_timepay);
		...
		Frame_RegistWorkChange.add(label_krw);
		
		label_main = new JLabel("출근부 수정");
		...
		Frame_RegistWorkChange.add(label_main);
		
		btn_regist = new JButton("수정");
		...
		Frame_RegistWorkChange.add(btn_regist);
		...
		Frame_RegistWorkChange.add(btn_cancel);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command == btn_regist) {
			...
			int workhour=0, workminute=0, worktime;
			...
			Oracle.OracleChangeWorkCalendar(Frame_Login.email, "workyear", combo_year.getSelectedItem().toString(), Frame_Calendar.combo_year.getSelectedItem().toString(), Frame_Calendar.combo_month.getSelectedItem().toString(), String.valueOf(Frame_Calendar.j+1));
			Oracle.OracleChangeWorkCalendar(Frame_Login.email, "workmonth", combo_month.getSelectedItem().toString(), combo_year.getSelectedItem().toString(), Frame_Calendar.combo_month.getSelectedItem().toString(), String.valueOf(Frame_Calendar.j+1));
			Oracle.OracleChangeWorkCalendar(Frame_Login.email, "workdate", combo_date.getSelectedItem().toString(), combo_year.getSelectedItem().toString(), combo_month.getSelectedItem().toString(), String.valueOf(Frame_Calendar.j+1));
			Oracle.OracleChangeWorkCalendar(Frame_Login.email, "worktime", String.valueOf(worktime), combo_year.getSelectedItem().toString(), combo_month.getSelectedItem().toString(), combo_date.getSelectedItem().toString());
			try {
			...
			new Frame_MessageBox("메시지", "출근부가 수정되었습니다.");
			Frame_Calendar.btn_date[Frame_Calendar.j+1].setBackground(Color.WHITE);
